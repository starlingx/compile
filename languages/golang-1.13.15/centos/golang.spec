...
%global go_api 1.13
%global go_tar_dir go-13.15
%global go_version 1.13.15
%global go_macro_version 1_13_15
%global obsoletes_name golang

# Fedora GOROOT
%global goroot          /usr/lib/%{name}-%{go_version}
%global gopath          %{_datadir}/gocode-%{go_version}
%global go_arches       %{ix86} x86_64 %{arm} aarch64 ppc64le
%global golibdir        %{_libdir}/golang-%{go_version}
...
URL:            http://golang.org/
Source0:        go%{go_version}.src.tar.gz
...
# another packages 'requires'
#
# Provides:       go = %{version}-%{release}
# Provides:       go = %{version}
# Provides:       golang = %{version}
# Provides:       go-srpm-macros = %{version}
Provides:       go-%{go_version}
Provides:       golang-%{go_version}
Provides:       go-srpm-macros-%{go_version}
Obsoletes:      %{obsoletes_name} < 1.13
...
Obsoletes:      %{obsoletes_name}-data < 1.1.1-4

# go1.4 deprecates a few packages
Obsoletes:      %{obsoletes_name}-vim < 1.4
Obsoletes:      emacs-%{obsoletes_name} < 1.4
...
Provides:       golang-go = %{version}-%{release}
Provides:       golang-go-%{go_version}
Requires:       %{name} = %{version}-%{release}
...
ln -sf %{goroot}/bin/go%{go_version} $RPM_BUILD_ROOT%{goroot}/bin/linux_%{gohostarch}/go
ln -sf %{goroot}/bin/gofmt%{go_version} $RPM_BUILD_ROOT%{goroot}/bin/linux_%{gohostarch}/gofmt
...
cp -av %{SOURCE100} $RPM_BUILD_ROOT%{_sysconfdir}/gdbinit.d/%{name}.gdb

# prelink blacklist
mkdir -p $RPM_BUILD_ROOT%{_sysconfdir}/prelink.conf.d
cp -av %{SOURCE101} $RPM_BUILD_ROOT%{_sysconfdir}/prelink.conf.d/golang-%{go_version}.conf
sed -e "s#/golang/#/golang-%{go_version}/#" \
    -i $RPM_BUILD_ROOT%{_sysconfdir}/prelink.conf.d/golang-%{go_version}.conf


# rpm macros
mkdir -p %{buildroot}
mkdir -p $RPM_BUILD_ROOT%{gopath}/src
cp -av %{SOURCE102} $RPM_BUILD_ROOT%{gopath}/src/macros.golang
sed -e "s#^\(.\)gopath\(.*\)#\1gopath \1{gopath_%{go_macro_version}}#" \
    -e "s#^\(.\)go_arches\(.*\)#\1go_arches \1{go_arches_%{go_macro_version}}#" \
    -i $RPM_BUILD_ROOT%{gopath}/src/macros.golang
%if 0%{?rhel} > 6 || 0%{?fedora} > 0
mkdir -p $RPM_BUILD_ROOT%{_rpmconfigdir}/macros.d
cp -av %{SOURCE102} $RPM_BUILD_ROOT%{_rpmconfigdir}/macros.d/macros.golang-%{go_version}
sed -e "s#^\(.\)gopath\(.*\)gocode#\1gopath_%{go_macro_version}\2gocode-%{go_version}#" \
    -e "s#^\(.\)go_arches#\1go_arches_%{go_macro_version}#" \
    -i $RPM_BUILD_ROOT%{_rpmconfigdir}/macros.d/macros.golang-%{go_version}
rm -f $RPM_BUILD_ROOT%{_rpmconfigdir}/macros.d/macros.golang
ln -sf /etc/alternatives/macros.golang $RPM_BUILD_ROOT%{_rpmconfigdir}/macros.d/macros.golang
%else
mkdir -p $RPM_BUILD_ROOT%{_sysconfdir}/rpm
cp -av %{SOURCE102} $RPM_BUILD_ROOT%{_sysconfdir}/rpm/macros.golang-%{go_version}
sed -e "s#^\(.\)gopath\(.*\)gocode#\1gopath_%{go_macro_version}\2gocode-%{go_version}#" \
    -e "s#^\(.\)go_arches#\1go_arches_%{go_macro_version}#" \
    -i $RPM_BUILD_ROOT%{_sysconfdir}/rpm/macros.golang-%{go_version}
rm -f $RPM_BUILD_ROOT%{sysconfdir}/rpm/macros.golang
ln -sf /etc/alternatives/macros.golang $RPM_BUILD_ROOT%{sysconfdir}/rpm/macros.golang
%endif
...
%if 0%{?rhel} > 6 || 0%{?fedora} > 0
%post
%{_sbindir}/update-alternatives --install %{_rpmconfigdir}/macros.d/macros.golang \
    macros.golang %{gopath}/src/macros.golang 90
%else
%post
%{_sbindir}/update-alternatives --install %{_sysconfdir}/rpm/macros.golang \
    macros.golang %{gopath}/src/macros.golang 90
%endif

%preun
if [ $1 = 0 ]; then
    %{_sbindir}/update-alternatives --remove macros.golang %{gopath}/src/macros.golang
fi
...
%{gopath}/src/macros.golang
%if 0%{?rhel} > 6 || 0%{?fedora} > 0
%{_rpmconfigdir}/macros.d/macros.golang-%{go_version}
%{_rpmconfigdir}/macros.d/macros.golang
%else
%{_sysconfdir}/rpm/macros.golang-%{go_version}
...
%changelog
* Mon Aug 31 2020 Alejandro Sáez <asm@redhat.com> - 1.13.15-1
- Rebase to go1.13.15

* Mon Jul 20 2020 Jakub Čajka <jcajka@redhat.com> - 1.13.14-1
- Rebase to go1.13.14
- Fix for CVE-2020-14040 and CVE-2020-15586
- Resolves: BZ#1853653, BZ#1856957

* Mon May 18 2020 Álex Sáez <asm@redhat.com> - 1.13.11-1
- Rebase to go1.13.11

* Mon Apr 20 2020 Jakub Čajka <jcajka@redhat.com> - 1.13.10-1
- Rebase to go1.13.10
